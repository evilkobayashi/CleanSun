...
Resolução padrão: 15 minutos. É um equilíbrio entre:
- granularidade visual suficiente para gráficos diários/semanais/mensais;
- tamanho controlado do `history.csv` para uso local e acadêmico.
"""
import argparse
import csv
import json
...
def actual_generation_kw(hour_float, weather, system_kwp, daily_factor):
    base = expected_generation_kw(hour_float, system_kwp)
    weather_gain = WEATHER_GAIN.get(weather, 0.76)
    noise = random.uniform(-0.06, 0.06)
    return max(0.0, base * weather_gain * daily_factor * (1.0 + noise))
...
def build_record(ts, interval_minutes, weather, tariff_kwh, system_kwp, total_generation_kwh):
    ...
    generation_kw = actual_generation_kw(hour_float, weather, system_kwp, day_factor)
...
def generate_history(days, interval_minutes, tariff_kwh, system_kwp, weather_mode="auto", end_ts=None):
    ...
    for ts in timestamps:
        ...
        weather = daily_weather[day_key]
        record, total_generation = build_record(ts, interval_minutes, weather, tariff_kwh, system_kwp, total_generation)
        records.append(record)
    return records, total_generation


class ContinuousSimulator:
    def __init__(self, host, port, interval_minutes, tariff_kwh, system_kwp, weather_mode, history_file, start_total):
        self.host = host
        self.port = port
    ...
    def _refresh_weather(self, ts):
        self.current_weather = pick_weather_for_day(int(ts) // 86400, self.weather_mode)

    def _append_current_record(self):
        ts = int(time.time())
        self._refresh_weather(ts)
        record, self.total_generation = build_record(
            ts,
        ...
        print(
            "[contínuo] ts={ts} clima={weather} geração={gen:.3f}kWh consumo={cons:.3f}kWh import={imp:.3f}kWh export={exp:.3f}kWh".format(
                ts=record["timestamp"],
                weather=record["weather_condition"],
                gen=record["solar_generation_kwh"],
                cons=record["house_consumption_kwh"],
                imp=record["grid_import_kwh"],
                exp=record["grid_export_kwh"],
            )
        )
...
def main():
    args = parse_args()
    records, total_generation = generate_history(
        days=args.days,
        interval_minutes=args.interval_minutes,
        tariff_kwh=args.tariff,
        system_kwp=args.system_kwp,
        weather_mode=args.weather,
    )
    write_history(records, args.history_file, overwrite=args.overwrite)
    print(
        "Histórico gerado: {days} dias, {count} registros, intervalo de {interval} min em {file}".format(
            days=args.days,
            count=len(records),
